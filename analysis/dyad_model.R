...
rm(list = ls())
setwd("/Users/erikbrockbank/web/vullab/rps-dyads/analysis")
...
DYAD_DATA_FILE = "../data/rps_v1_data.csv" # name of file containing full dyad dataset
...
# Get player win count differentials, top quartile participants by win count differential
...
# Sanity check: look at data among folks with positive win counts
# win_diff %>% filter(win_count >= THRESHOLD) %>% nrow()
...
# MODEL ANALYSIS - HIGH WIN COUNTS (UNWEIGHTED) ====
# NB: re-run graphs above but filtering for participants in top quartile WCD
# to see how well the model fits among the best performing participants.
# Next, run the belo to see the overall relationship between
# task performance and model fits.
...
# Correlation between log likelihood and win count differentials
...
cor.test(fit_summary_transitions_cor$ll_per_round, fit_summary_transitions_cor$win_count) # p = .004
...
plot(fit_summary_moves_cor$softmax, fit_summary_moves_cor$win_count)
plot(fit_summary_transitions_cor$softmax, fit_summary_transitions_cor$win_count)
...
cor.test(fit_summary_transitions_cor$softmax, fit_summary_transitions_cor$win_count) # N.S.
...
# NB: re-run graphs above but filtering for participants in top quartile WCD
# to see how well the model fits among the best performing participants.
# Next, run the below to see the overall relationship between
# task performance and model fits.

fit_summary_moves_cor_weighted = fit_summary_moves_weighted %>%
  inner_join(win_diff, by = c('subject')) %>%
  filter(win_count >= 0)
fit_summary_transitions_cor_weighted = fit_summary_transitions_weighted %>%
  inner_join(win_diff, by = c('subject')) %>%
  filter(win_count >= 0)
fit_summary_opponent_transitions_cor_weighted = fit_summary_opponent_transitions_weighted %>%
  inner_join(win_diff, by = c('subject')) %>%
  filter(win_count >= 0)
fit_summary_move_prev_move_cor_weighted = fit_summary_move_prev_move_weighted %>%
  inner_join(win_diff, by = c('subject')) %>%
  filter(win_count >= 0)
fit_summary_move_opponent_prev_move_cor_weighted = fit_summary_move_opponent_prev_move_weighted %>%
  inner_join(win_diff, by = c('subject')) %>%
  filter(win_count >= 0)
fit_summary_outcome_transition_cor_weighted = fit_summary_outcome_transition_weighted %>%
  inner_join(win_diff, by = c('subject')) %>%
  filter(win_count >= 0)


# Correlation between log likelihood and win count differentials
plot(fit_summary_moves_cor_weighted$ll_per_round, fit_summary_moves_cor_weighted$win_count)
plot(fit_summary_transitions_cor_weighted$ll_per_round, fit_summary_transitions_cor_weighted$win_count)
plot(fit_summary_opponent_transitions_cor_weighted$ll_per_round, fit_summary_opponent_transitions_cor_weighted$win_count)
plot(fit_summary_move_prev_move_cor_weighted$ll_per_round, fit_summary_move_prev_move_cor_weighted$win_count)
plot(fit_summary_move_opponent_prev_move_cor_weighted$ll_per_round, fit_summary_move_opponent_prev_move_cor_weighted$win_count)
plot(fit_summary_outcome_transition_cor_weighted$ll_per_round, fit_summary_outcome_transition_cor_weighted$win_count)


cor.test(fit_summary_moves_cor_weighted$ll_per_round, fit_summary_moves_cor_weighted$win_count) # p = .004
cor.test(fit_summary_transitions_cor_weighted$ll_per_round, fit_summary_transitions_cor_weighted$win_count) # p = .0001
cor.test(fit_summary_opponent_transitions_cor_weighted$ll_per_round, fit_summary_opponent_transitions_cor_weighted$win_count) # p = .003
cor.test(fit_summary_move_prev_move_cor_weighted$ll_per_round, fit_summary_move_prev_move_cor_weighted$win_count) # p = .0004
cor.test(fit_summary_move_opponent_prev_move_cor_weighted$ll_per_round, fit_summary_move_opponent_prev_move_cor_weighted$win_count) # p = .0008
cor.test(fit_summary_outcome_transition_cor_weighted$ll_per_round, fit_summary_outcome_transition_cor_weighted$win_count) # p < .0001



# Correlation between softmax estimates and win count differentials
plot(fit_summary_moves_cor_weighted$softmax, fit_summary_moves_cor_weighted$win_count)
plot(fit_summary_transitions_cor_weighted$softmax, fit_summary_transitions_cor_weighted$win_count)
plot(fit_summary_opponent_transitions_cor_weighted$softmax, fit_summary_opponent_transitions_cor_weighted$win_count)
plot(fit_summary_move_prev_move_cor_weighted$softmax, fit_summary_move_prev_move_cor_weighted$win_count)
plot(fit_summary_move_opponent_prev_move_cor_weighted$softmax, fit_summary_move_opponent_prev_move_cor_weighted$win_count)
plot(fit_summary_outcome_transition_cor_weighted$softmax, fit_summary_outcome_transition_cor_weighted$win_count)


cor.test(fit_summary_moves_cor_weighted$softmax, fit_summary_moves_cor_weighted$win_count) # N.S.
cor.test(fit_summary_transitions_cor_weighted$softmax, fit_summary_transitions_cor_weighted$win_count) # p = .004
cor.test(fit_summary_opponent_transitions_cor_weighted$softmax, fit_summary_opponent_transitions_cor_weighted$win_count) # p = .005
cor.test(fit_summary_move_prev_move_cor_weighted$softmax, fit_summary_move_prev_move_cor_weighted$win_count) # p = .02
cor.test(fit_summary_move_opponent_prev_move_cor_weighted$softmax, fit_summary_move_opponent_prev_move_cor_weighted$win_count) # p = .11
cor.test(fit_summary_outcome_transition_cor_weighted$softmax, fit_summary_outcome_transition_cor_weighted$win_count) # p < .0001
...
# MODEL COMPARISON - WEIGHTED MOVE BASERATES ====
...
# MODEL COMPARISON - WEIGHTED TRANSITIONS ====
...
# MODEL COMPARISON - WEIGHTED OPPONENT TRANSITIONS ====
...
# MODEL COMPARISON - WEIGHTED MOVE GIVEN PREVIOUS MOVE ====
...
# MODEL COMPARISON - WEIGHTED MOVE GIVEN OPPONENT PREVIOUS MOVE ====
...
# MODEL COMPARISON - WEIGHTED TRANSITION GIVEN PREVIOUS OUTCOME ====

# Weighted transition given previous outcome
# NB: this takes ~8 mins.
